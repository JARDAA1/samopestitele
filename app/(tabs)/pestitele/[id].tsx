...
export default function PestitelDetail() {
  ...
  const handleAddToCart = (product) => {
    addToCart(product, pestitel);
    // Jednoduchá zpětná vazba - produkt přidán
    Alert.alert(
      "✅ Přidáno do košíku",
      `${product.nazev} (${product.cena} Kč)`
    );
  };
        ...
        <>
          ...
          {pestitel.produkty.map((prod, index) => (
            <View key={index} style={styles.productRow}>
              ...
              >
                <Text style={styles.addBtnText}>+ Do košíku</Text>
              </TouchableOpacity>
            </View>
          ))}
        </>
...
const styles = StyleSheet.create({
  ...
  eventDesc: { color: "#333", marginTop: 4 },
});
